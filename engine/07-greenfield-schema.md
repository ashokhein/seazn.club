...
4. **Public read model** — the dashboard/API reads only:
   competitions(visibility in public/unlisted — doc 09 §1: unlisted is link-only, the
   page renders noindex; private never leaves the views), divisions, stages, pools,
   fixtures(status, outcome, schedule), standings_snapshots, entrants(display), lineups
   filtered by consent. Implemented as SQL views (`public_fixtures_v`, …) so the
   consent/privacy filtering (doc 06 §4.7) lives in exactly one place; `branding` is
   nulled in-view for orgs without the Pro branding entitlement (doc 10 §2 rule 3,
   `org_has_feature()`).
...
- **Public read model** — eight views (`public_competitions_v`,
  `public_divisions_v`, `public_stages_v`, `public_pools_v`, `public_fixtures_v`,
  `public_standings_v`, `public_entrants_v`, `public_players_v`) plus a
  ...
  DOB never selected. `public_players_v` (the player-card source, PROMPT-12)
  contains ONLY name-consented persons rostered in a publicly visible
  competition — the card 404s for everyone else; entrant member `person_id` is
  exposed only with name consent (it is the card's link target).
