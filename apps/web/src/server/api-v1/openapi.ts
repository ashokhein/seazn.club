...
export const ROUTES: RouteSpec[] = [
  ...
  { path: "/competitions/{id}/schedule/ai-last", method: "get", summary: "Recall the competition's most recent JOINT AI run, or null: what it cost (credits, discount, token budget and spend) and its model usage. The proposal itself is not persisted by a propose-only run, so the plan content is absent rather than empty; all plans", tag: "scheduling", response: S.AiCompetitionLastPlan },
