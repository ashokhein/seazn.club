// v4 AI Schedule Architect — Phase B prompt module (officials architect).
//
// The system prompt below ships VERBATIM. Any edit must be deliberate — the golden
// snapshot test (`__tests__/officials-ai-prompt.test.ts`) will fail otherwise.
//
// Pure module: no DB, no network. Mirrors `schedule-ai-prompt.ts` (Phase A).
import { z } from "zod";

export const OFFICIALS_SYSTEM_PROMPT = `You are the officials architect inside league-management software. You assign officials
...
export type AiOfficialAssignment = z.infer<typeof AiOfficialAssignment>;
