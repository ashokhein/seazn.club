...
export async function GET(req: Request, { params }: Ctx) {
  return v1(async () => {
    ...
    return listSponsorPackages(auth);
  });
}
...
export async function POST(req: Request, { params }: Ctx) {
  return v1(async () => {
    const { id } = await params;
    assertUuid(id, "organization");
    const body = await parseBody(req, CreateSponsorPackageInput);
    const auth = await requireOrgAuth(req, id, "write");
    return reply(201, await createSponsorPackage(auth, body));
  });
}
