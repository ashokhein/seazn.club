...
**An extra seat stops you adding members, and marks the ones over the limit.** Cancelling a seat removes nobody and touches no data. What it stops outright is *growing*: an invitation or a promotion that would take you past the limit is refused, everywhere, straight away. Members already over the limit are marked read-only — the same idea as [what downgrading freezes](/help/billing/downgrade) — and owners are never marked. Be aware that read-only mark is enforced on our public API today rather than on every screen in the app, so treat it as a signal to make room rather than as a lock. Buying the seat back clears both immediately.
