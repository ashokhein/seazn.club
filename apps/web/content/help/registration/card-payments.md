...
A card sponsorship payment runs on the same Connect rail, so a disputed sponsorship charge follows the same path. The moment Stripe tells us, the sponsor's order is flagged **Disputed** in your monetization console, its **placement comes down** — boards and public pages stop showing it, and the sponsor list marks it **Disputed — parked**. You can't switch a parked placement back to active yourself; it returns automatically if the dispute is won. You also get an alert. Win the dispute and the placement goes back up automatically; lose it and the order is written off, the placement stays down, and the amount is **recovered from your Stripe balance**. Seazn Club absorbs Stripe's dispute fee here too, and you're emailed what moved.
