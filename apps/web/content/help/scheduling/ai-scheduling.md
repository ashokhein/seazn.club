...
Press undo twice in a row and the second press is refused rather than queued, because the first is still running — wait a moment and try again. If the competition has been applied again since, undo has nothing to put back and says so instead of rewinding the newer schedule.
