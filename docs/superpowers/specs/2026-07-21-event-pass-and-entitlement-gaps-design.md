...
| D18 | **`branding` opens to community.** Every org may upload a logo. The key stays in `plan_entitlements` and every gate call stays in place — only the community value flips to `true`, so nothing breaks and the key drops out of the pass's lifted set. `dashboard.branding` (org theme colour on public pages and the slideshow) **stays Pro-only** and stays denied to the pass — logos become table stakes, theme colour remains the visible Pro differentiator. |
| D19 | **`registration.paid` opens to community**, monetised by fee rather than by gate. New ladder for `registration.fee_percent`: **community 8, event_pass 5, pro 2, pro_plus 1.** Community needs an EXPLICIT row — today it has none and falls back to `PLATFORM_FEE_PERCENT` (default 5), which would leave the pass with no fee advantage at all. |
| D20 | The pass's fee story changes from "unlock entry fees" to "**cheaper** entry fees" — 8% → 5%, saving 3 points. On a £500 event that is £15, so a $29 pass pays for itself around £970 of entries. All pass copy must stop claiming entry fees are a pass-only feature (Task 18). |
| D21 | **Every surface that describes plans, prices, fees or pass contents is in scope — not just marketing copy.** That explicitly includes the **admin panel**, help content, the Stripe product description, emails, exported documents, and all four locales. Task 18 is widened from four surfaces to whatever an audit finds, and the audit output becomes its checklist. A surface is only "done" when it matches the live `plan_entitlements` matrix. |
| D22 | **The marketing was the intent; the matrix was wrong.** Community rises to `entrants.per_division.max` **32** (advertised as "32 players") and `competitions.max_active` **5** (advertised as "5 seasons"). The Event Pass rises to **64** entrants so it keeps a real lift — ladder 32 free / 64 pass / 256 Pro / ∞ Plus. Passed competitions are already excluded from the active count (`competitions.ts:86`), so a pass still adds capacity beyond the cap. |
| D23 | **`branding` vs `dashboard.branding` must gate different controls.** Logo upload stays free on `branding`; the brand-colour picker moves to `dashboard.branding` (Pro). V310 left the picker offered to Community while the colour is stripped at render (`public-site/data.ts:180`) — a control that silently does nothing. This revives the dead `settings.upgrade.brandColor` upsell string. |
| D24 | The `/pricing` fee row is relabelled **"Platform fee on entry fees"**. The old label — "Entry fees paid out to your club" — now sits beside `✓ 8%` and reads as the exact inverse of the truth. |
