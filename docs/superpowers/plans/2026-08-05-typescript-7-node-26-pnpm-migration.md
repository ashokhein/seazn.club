...
- **All three stages land on the long-lived `typescript7` base branch, never directly on `main`.** `main` is touched once, at the end, by a single `typescript7 -> main` PR raised after everything is verified locally. `typescript7` was created at `6847b3c6`.
  ...
  - **Owner's model: stages merge LOCALLY into `typescript7` (fast-forward, no per-stage PR). The only PR in the programme is `typescript7 -> main`, raised once everything is verified locally.** The `gh pr create --base typescript7` block at the end of each stage is therefore optional — use it only if you want CI to exercise one stage in isolation; otherwise its body text becomes the merge description.
  - **Consequence of that choice, and it is the sharp one:** smoke CI runs on **PRs only**. A stage merged locally gets no CI smoke at all, so the local smoke run is the only one that stage will ever have. Never skip a local smoke on the grounds that "CI will catch it" — here it will not, until the final integration PR.
  - **Provision smoke and e2e from the workflow `env:` blocks, never from memory.** Measured on stage 1: an under-provisioned smoke reported `710 passed, 0 failed` with the entire v4 AI section silently skipped; the authoritative env gave **763**. The +53 was the hole. Sources: the `smoke` job's `env:` in `ci.yml`, and the `env:` block in `e2e.yml`.
