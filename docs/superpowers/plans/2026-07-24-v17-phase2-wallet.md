...
- [x] Task 5 review (2026-07-24): confirmed `officials.auto` stays in place — it still gates 3 `requireFeature` call sites in `usecases/officials.ts` (the manual/solver-auto path), so it is NOT vestigial overall; only the AI-officials path (`officials-ai.ts`, Task 4) was un-gated from it.
- [ ] commit + **push**.
