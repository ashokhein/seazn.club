...
**Read first:** `v3/10-roi-features-and-suggestions.md` Wave 1 (normative); `v3/03` §5
(logo matrix), `engine/Jul3/06` (DocModel for the poster), `engine/09` (public dashboard).
Preamble: PROMPT-00. **Depends:** PROMPT-32 (logos, chips).
...
1. **OG/share images:** satori+resvg renderer (self-contained, no external fonts at
   ...
   cached (ISR-aligned revalidation).
...
- Golden: OG renderer snapshot per template (deterministic fonts); QR resolves to the
  public URL; embed 404s a private division and renders a link-only one.
