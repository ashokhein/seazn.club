...
§3 (bottom-sheet modal). Preamble: PROMPT-00. **Depends:** PROMPT-30 (route names),
PROMPT-32 (primitives — build §3 sheet variant there first or here, don't build twice).
...
6. **Viewport gate** (v3/02 §4): Playwright `mobile` project (375×667, 390×844),
   `expectNoHorizontalScroll` on every audited route, page smokes (settings save, invoice
   render, score sheet submit); wire into CI smoke job.
