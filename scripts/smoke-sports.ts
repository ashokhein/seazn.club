...
async function v1(s: Session, path: string, method = "GET", body?: unknown): Promise<V1Res> {
...
async function setPlan(orgId: string, plan: string): Promise<void> {
  const sql = db();
  try {
    // Billing lives on the GROUP (V310) — see setPlan in scripts/smoke.ts.
...
const DRIVERS: SportDriver[] = [
  {
    ...
    async edge(s, fx, label) {
      ...
      check(
        `${label}: level score → draw outcome`,
        st.outcome?.kind === "draw",
        JSON.stringify(st.outcome),
      );
      const dupe = await append(
        s,
        fx.id,
    ...
    async edge(s, fx, label) {
      let seq = await appendAll(
        ...
        [
          ...
          {
            type: "football.card",
            payload: { by: fx.away, color: "red", minute: 30 },
          },
          // Own goal: struck by home, credited to away → 1-1 at FT.
          {
            type: "football.goal",
            payload: { by: fx.home, ownGoal: true, minute: 44 },
          },
          { type: "football.period", payload: { phase: "HT" } },
          { type: "football.period", payload: { phase: "FT" } },
        ],
      ...
      check(
        `${label}: drawn FT with shootout cfg stays undecided`,
        st.outcome === null,
        JSON.stringify(st.outcome),
      );
          ...
          [
            {
              ...
              payload: { by: fx.home, scored: true },
            },
            {
              type: "football.shootout.kick",
              payload: { by: fx.away, scored: false },
            },
          ],
          seq,
        );
  ...
  },
  {
    ...
    async edge(s, fx, label) {
      ...
      check(
        `${label}: summary before start → 422 WRONG_PHASE`,
      ...
      const seq = await appendAll(
        ...
        [
          ...
          {
            type: "cricket.innings.summary",
            payload: { runs: 80, wickets: 2, legalBalls: 60, partial: true },
          },
          {
            type: "cricket.interruption",
            payload: { kind: "rain", oversLostEstimate: 5 },
          },
          { type: "cricket.innings.close", payload: {} },
          {
            type: "cricket.revise",
            payload: { oversPerSide: 15, target: 100 },
          },
          // Chasing side (home) reaches the revised target inside the revised overs.
          {
            type: "cricket.innings.summary",
            payload: { runs: 100, wickets: 4, legalBalls: 78 },
          },
        ],
        0,
      );
  ...
  },
  {
    ...
    async edge(s, fx, label) {
      ...
      const dupe = await append(
        s,
        fx.id,
        { type: "boardgame.result", payload: { winner: fx.home } },
        seq,
      );
  ...
  },
  {
    ...
    async edge(s, fx, label) {
      ...
      let seq = await appendAll(
        s,
        fx.id,
        [{ type: "carrom.toss", payload: { firstBreak: fx.away } }, start],
        0,
      );
      let st = await state(s, fx.id);
  ...
  },
  {
    ...
    async edge(s, fx, label) {
      ...
      let seq = await appendAll(
        s,
        fx.id,
        [start, { type: "volleyball.set.summary", payload: { home: 25, away: 20 } }],
        0,
      );
      ...
      seq = await appendAll(
        ...
        [
          {
            type: "volleyball.set.summary",
            payload: { home: 10, away: 8, partial: true },
          },
          {
            type: "volleyball.set.summary",
            payload: { home: 15, away: 12, partial: true },
          },
        ],
        seq,
      );
      const dec = await append(
        s,
        fx.id,
        {
          type: "volleyball.set.summary",
          payload: { home: 9, away: 8, partial: true },
        },
        seq,
      );
      ...
      seq = await appendAll(
        ...
        Array.from({ length: 10 }, () => ({
          ...
          payload: { wonBy: fx.home },
        })),
        seq,
      );
      let st = await state(s, fx.id);
      check(
        `${label}: rallies close the snapshot set (2-0, undecided)`,
        st.outcome === null,
        `outcome ${JSON.stringify(st.outcome)}`,
      );
      // Set 3: an unreachable final score is rejected, then a real one wins 3-0.
      const bogus = await append(
        s,
        fx.id,
        {
          type: "volleyball.set.summary",
          payload: { home: 25, away: 24 },
        },
        seq,
      );
      check(
        `${label}: unreachable set score (25-24) → 422`,
        bogus.status === 422,
        `got ${bogus.status}`,
      );
      await appendAll(
  ...
  },
  {
    ...
    async edge(s, fx, label) {
      ...
      const st = await state(s, fx.id);
      check(
        `${label}: abandon → status abandoned, no outcome`,
        st.status === "abandoned" && st.outcome === null,
        `status ${st.status} outcome ${JSON.stringify(st.outcome)}`,
      );
      const late = await append(
        s,
        fx.id,
        { type: "tabletennis.game.summary", payload: { home: 11, away: 0 } },
        seq,
      );
...
async function runSport(s: Session, compId: string, d: SportDriver): Promise<void> {
  ...
  const fx: Fx[] = rows.fixtures.map((f) => ({
    id: f.id,
  ...
  check(
    `${label}: scoring before division start → 422 WRONG_PHASE`,
    closed.status === 422 && closed.json.error?.code === "WRONG_PHASE",
    `got ${closed.status} ${closed.json.error?.code}`,
  );
  ...
  // f2 — forfeit: away forfeits → award to home.
  {
    await appendAll(
      ...
      [
        ...
        {
          type: "core.forfeit",
          payload: { by: fx[2].away, reason: "no-show" },
        },
      ],
      0,
    );
  ...
  // f3 — optimistic concurrency: parallel scorers race on the same seq.
  {
    ...
    check(
      `${label}: parallel scorers → one 201, one 409`,
      won.length === 1 && lost.length === 1,
      race.map((r) => r.status).join("/"),
    );
    check(
      `${label}: 409 carries current_seq`,
      lost[0]?.json.error?.current_seq === seq + 1,
      String(lost[0]?.json.error?.current_seq),
    );
    await decideFrom(s, d, fx[3], seq + 1);
    const st = await state(s, fx[3].id);
    check(
      `${label}: decided after conflict recovery`,
      st.outcome?.kind === "win",
      JSON.stringify(st.outcome),
    );
  }
  ...
  // f4 — undo: void a note, then decide normally.
  {
    ...
    const events = data<{ id: string; seq: number }[]>(
      await must(s, `/api/v1/fixtures/${fx[4].id}/events`),
    );
    const target = events.find((e) => e.seq === seq);
    const voided = await append(
      s,
      fx[4].id,
      { type: "core.void", payload: { event_id: target?.id } },
      seq,
    );
    ...
    check(
      `${label}: decided after void`,
      st.outcome?.kind === "win" && st.outcome.winner === fx[4].home,
      JSON.stringify(st.outcome),
    );
  }
  ...
  // f5 — idempotent retry, then finalize locks the ledger.
  {
    const first = await v1(s, `/api/v1/fixtures/${fx[5].id}/events`, "POST", {
      ...
      type: "core.start",
      payload: {},
      idempotency_key: `smoke-${tag}-${d.sportKey}`,
    });
    ...
    const replayed =
      retry.status === 201 && data<{ seq: number }>(first).seq === data<{ seq: number }>(retry).seq;
    ...
    const fin = await v1(s, `/api/v1/fixtures/${fx[5].id}/finalize`, "POST", {
      expected_seq: st.last_seq,
    });
    check(`${label}: finalize`, fin.json.ok === true, `got ${fin.status}`);
    const after = await append(s, fx[5].id, note);
    check(
      `${label}: append after finalize rejected`,
      after.status === 422 || after.status === 403,
      `got ${after.status}`,
    );
  }
  ...
  check(
    `${label}: standings rank all 4 entrants`,
    data<{ rows: unknown[] }>(standings).rows.length === 4,
  );
}
...
async function communityGateSuite(): Promise<void> {
  ...
  const coarse = await append(
    s,
    f.id,
    { type: "volleyball.set.summary", payload: { home: 25, away: 12 } },
    1,
  );
  check(
...
async function main() {
  ...
  for (const driver of DRIVERS) {
    try {
      ...
      check(
        `${driver.sportKey}/${driver.variantKey}: suite completed`,
        false,
