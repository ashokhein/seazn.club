...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = '${flyway:defaultSchema}' AND tablename = 'audit_log') THEN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = '${flyway:defaultSchema}' AND tablename = 'audit_log_v1') THEN
      ...
      EXECUTE 'SELECT count(*) FROM audit_log' INTO fresh_rows;
      IF fresh_rows > 0 THEN
        RAISE EXCEPTION '013_v1_cutover: audit_log has rows but audit_log_v1 archive already exists — resolve manually';
      END IF;
    ...
    END IF;
  END IF;
  -- Rename the archive's indexes too: schema.sql re-creates audit_log (and
  -- audit_log_tournament_idx) on a bootstrap re-run, which would otherwise
  -- collide with the archived table's index names.
  IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
             WHERE c.relname = 'audit_log_tournament_idx'
               AND i.indrelid = 'audit_log_v1'::regclass) THEN
    ALTER INDEX audit_log_tournament_idx RENAME TO audit_log_v1_tournament_idx;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
             WHERE c.relname = 'audit_log_org_idx'
               AND i.indrelid = 'audit_log_v1'::regclass) THEN
    ALTER INDEX audit_log_org_idx RENAME TO audit_log_v1_org_idx;
  END IF;
  IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_index i ON i.indexrelid = c.oid
             WHERE c.relname = 'audit_log_chain_idx'
               AND i.indrelid = 'audit_log_v1'::regclass) THEN
    ALTER INDEX audit_log_chain_idx RENAME TO audit_log_v1_chain_idx;
  END IF;
END $$;
