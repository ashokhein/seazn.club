...
| #391 | B | 4 | **IN FLIGHT** — dispatched with 4+5 as ONE pass (both edit `help-copy-truth.test.ts`). Brief carries: the limit must be read out of `competition-schedule-ai.ts:1660`, not the plan prose; help tree is English-only, owes no i18n; test bar is unit + regression only. |
| #392 | B | 5 | **IN FLIGHT** — same pass. Brief flags the highest-value risk: a corpus-scanning guard that finds nothing is indistinguishable from one that CANNOT find anything, so fire-tests against a known-bad string are mandatory. Also told to hunt help copy asserting the OLD per-division undo loop, which tasks 2/2b/2c/3 have made false. |
...
| #382 | E | 12, 13, 14, 15 | **IN FLIGHT** — implementer dispatched 2026-08-05 in `wt-ai-gap-e` off A's tip. Brief carries the V344→**V353** correction, PG 54353, and the mutation-proof requirement. |
